...
// the [backend.CallParams] map with direct, statically typed field
// assignments, an unmarshalOut helper reads it back with explicit
// type assertions, and a CallFast wrapper drives the call via
// [nwrfc.CallMap]. No "reflect" import appears anywhere in the
// generated file, so the per-call reflection cost paid inside
// [nwrfc.Call] is eliminated on the hot path. The measurable win is
// performance, not just typing.
//
...
// reflection-free marshal/unmarshal path: marshalIn builds the
// backend.CallParams map with direct field assignments and
// unmarshalOut reads it back with explicit type assertions. No
// "reflect" import appears anywhere in the output, so the
// per-call reflection cost paid inside nwrfc.Call is eliminated
// on the hot path. The win is throughput, not just typing.
...
// backend.CallParams map. It is reflection-free: every field is a
// direct, statically typed assignment, so the per-call reflection
// cost paid inside nwrfc.Call is eliminated here.
func marshalIn(in In) backend.CallParams {
	raw := make(backend.CallParams, {{len .FastIn}})
{{- range .FastIn}}
...
// unmarshalOut reads a backend.CallParams map into the typed
// output struct. It is reflection-free: every field is read
